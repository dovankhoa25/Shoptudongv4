...
class NroReceivingService
{
    public function start(User $user, int $id, array $v): int
    {
        return DB::transaction(function () use ($user, $id, $v) {
            ...
            $o = DB::table('item_orders')->where('id', $id)->where('buyer_id', $user->id)->first(); abort_unless($o, 404);
            ...
            $session = DB::table('nro_delivery_sessions')->insertGetId([
                'order_id' => $id, 'request_key' => $v['requestKey'], 'mode' => $v['mode'], 'status' => 'queued',
                'trade_in_flight' => false,
                'recipient_name' => $v['mode'] === 'manual' ? $v['recipientName'] : null,
                'receiver_credentials' => $credentials, 'receiver_lock' => $lock, 'wait_minutes' => $a->wait_minutes,
    ...
    public function retryInterrupted(object $job): bool
    {
        ...
        DB::table('item_orders')->where('id', $job->order_id)->update([
            'status' => 'awaiting_receipt', 'delivery_message' => 'Phiên nhận đã dừng trước khi giao dịch hoặc sau lượt đã xác nhận. Đồ còn lại vẫn được giữ; bấm Nhận đồ để tiếp tục, không thanh toán lại.', 'updated_at' => now(),
        ]);
        $this->finish($job, 'failed');
        DB::table('nro_worker_jobs')->where('id', $job->id)->update([
            'status' => 'failed', 'result_json' => json_encode(['retrySafe' => true, 'message' => 'Stopped outside an unconfirmed trade.']), 'updated_at' => now(),
        ]);
        return true;
    }
    ...
    public function finish(object $job, string $status): void
    {
        ...
        DB::table('nro_delivery_sessions')->where('id', $job->delivery_session_id)->update([
            'status' => $status, 'receiver_credentials' => null,
            // Keep the login lock on uncertain results until reconciliation.
            'receiver_lock' => $status === 'review' ? DB::raw('receiver_lock') : null, 'updated_at' => now(),
        ]);
    }
}
