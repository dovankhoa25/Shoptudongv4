...
- Nhóm sidebar **Quản lý IP** gồm bốn trang riêng: **IP dùng chung**, **Duyệt thiết bị**, **Chặn IP / dải IP**, **Lịch sử đăng nhập**. Nút Quản lý IP trong danh sách người dùng cũng dẫn đến nhóm này. Các trang và API chi tiết yêu cầu `access-security.manage`.
- **IP dùng chung** (`/admin/ip-management/overview`) mặc định hiển thị IP có ít nhất hai tài khoản đăng nhập thành công trong 30 ngày, sắp theo số tài khoản rồi hoạt động gần nhất. Lọc 1/7/30/90 ngày, IP/username/#ID hoặc chuyển sang tất cả IP. Tìm một tài khoản vẫn giữ các tài khoản khác dùng chung IP đó. Chỉ hiển thị ba tài khoản xem trước mỗi IP; bảng chi tiết phân trang 20 tài khoản và có 10 lần đăng nhập gần nhất. Lần đăng nhập thất bại không được dùng để kết luận tài khoản đã sử dụng IP. IP dùng chung không chứng minh cùng chủ sở hữu; cần đối chiếu nguồn IP và lịch sử trước khi chặn.
- **Duyệt thiết bị**: mặc định chờ duyệt; có lọc đã duyệt, hết hạn, thu hồi, tìm IP/tài khoản và duyệt/thu hồi ngay trong bảng. **Chặn IP / dải IP**: mặc định các lệnh đang hiệu lực, thêm lịch sử hết hạn/đã mở chặn; có thể mở form chặn từ tổng quan hoặc chi tiết IP. Form giữ kiểm tra không tự chặn IP quản trị và không tự suy ra CIDR. **Lịch sử đăng nhập**: lọc IP/tài khoản, thành công/thất bại và khoảng thời gian, hiển thị nguồn IP cùng User-Agent. Link quản lý tài khoản chỉ hiện khi có thêm `users.view`.
- Trong trang **Duyệt thiết bị**, công tắc **Duyệt đăng nhập admin** bật/tắt việc yêu cầu duyệt IP và trình duyệt mới. Khi bật từ giao diện, IP và trình duyệt hiện tại của người bật được duyệt để tránh tự khóa; các phiên khác chưa được duyệt bị chặn từ yêu cầu tiếp theo. Tắt không xóa lịch sử/yêu cầu duyệt và không tắt kiểm tra mật khẩu, khóa tài khoản hoặc chặn IP tại HTTP. Mọi lần thay đổi có sự kiện `admin_access_policy_changed` ghi người thao tác, IP và trạng thái trước/sau.
- Các trang mới chỉ truy vấn khi mở/lọc/chuyển trang hoặc chủ động làm mới, không thêm vòng polling. Có phân trang 20/50/100 dòng, thời gian tổng hợp tối đa 90 ngày và dùng index `created_at`/`ip_address` hiện có. Các đường chặn/mở chặn tiếp tục dùng cache IP và invalidation hiện hành. Phần giao diện này không thêm migration; cần cập nhật source, build `public/build` và route cache khi triển khai. Chưa đo thời gian truy vấn trên dữ liệu production.
