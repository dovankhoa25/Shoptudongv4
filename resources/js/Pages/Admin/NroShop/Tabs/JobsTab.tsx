        ...
        <>
            ...
            <Table
                ...
                columns={[
                    ...
                    { title: 'Kết quả', render: (_, j: Job) => <>{jobResult(j)}{!!j.rounds?.length && <details className="mt-2 text-xs"><summary className="cursor-pointer text-sky-600">Lịch sử {j.rounds.length} lượt giao</summary>{j.rounds.map(round=><div key={round.id} className="mt-2 rounded border p-2"><strong>Lượt #{round.id} · {({started:'Đã lưu trước giao',confirmed:'Đã xác nhận giao',cancelled:'Đã hủy',recovered:'Đã kiểm tra khôi phục'} as Record<string,string>)[round.status] || round.status}</strong><div>{dateTime(round.startedAt)}</div>{round.items.map(item=><div key={item.id}>Dòng #{item.id}: kho trước {item.before} · giao lượt này {item.offered} · đã giao trước đó {item.delivered}</div>)}<div className="break-all text-slate-500">{round.key}</div></div>)}</details>}{!!j.lateResults?.length && <details className="mt-2 text-xs"><summary className="cursor-pointer text-amber-500">Có kết quả tool gửi sau khi chốt · Xem để đối chiếu</summary>{j.lateResults.map((x,i)=><pre key={i} className="max-h-60 max-w-md overflow-auto whitespace-pre-wrap">{x.at} · {x.kind}{'\n'}{JSON.stringify(x.data,null,2)}</pre>)}</details>}</> },
                    {
                        title: '',
                        width: 110,
                        render: (_, j: Job) =>
